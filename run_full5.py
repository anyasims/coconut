...
class Trainer:
    def __init__(self, cfg) -> None:
        ...
        # answer prompt
        if cfg.dataset.endswith("_hash"):
            self.answer_prompt_ids = [self.tokenizer.encode("####")]
        else:
            self.answer_prompt_ids = [self.tokenizer.encode("Answer:"), self.tokenizer.encode(" Answer:")]
        ...
        if self.rank == 0:
            ...
            print(f"Answer prompts: {[self.tokenizer.decode(ap) for ap in self.answer_prompt_ids]}")
    ...
    def set_config(self, cfg):
        ...
        if self.rank == 0:
            ...
            run_name += f"-hash" if cfg.dataset.endswith("_hash") else ""
    ...
    def find_answer_prompt(self, response_ids):
        if len(self.answer_prompt_ids) == 1 and len(self.answer_prompt_ids[0]) == 1:
            ap_id = self.answer_prompt_ids[0][0]
            matches = (response_ids == ap_id)
            contains_answer_prompt = matches.any()
            if contains_answer_prompt:
                answer_prompt_index = matches.nonzero(as_tuple=True)[0][0]
            else:
                answer_prompt_index = None
            return contains_answer_prompt, answer_prompt_index
        else:
            contains_answer_prompt = False
            earliest_end_idx = len(response_ids) + 1
            for ap in self.answer_prompt_ids:
                ap = torch.tensor(ap, device=self.device)
                windows = response_ids.unfold(0, len(ap), 1)
                matches = (windows == ap).all(dim=1)
                if matches.any():
                    contains_answer_prompt = True
                    # Get the index of the first match for this candidate.
                    idx = matches.nonzero(as_tuple=False)[0].item()
                    end_idx = idx + len(ap) - 1  # final index of the matching window
                    earliest_end_idx = min(earliest_end_idx, end_idx)
            return contains_answer_prompt, earliest_end_idx
    ...
    def generate(
        ...
        for i, (response_ids, answer_ids) in enumerate(zip(responses_ids, tokenized_answers)):
            ...
            max_ap_length = max(len(ap) for ap in self.answer_prompt_ids)
            contains_ap, response_ap_idx = self.find_answer_prompt(response_ids[:-answer_length-max_ap_length])
            contains_answer_prompt[i] = int(contains_ap)
